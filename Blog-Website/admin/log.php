...
if ($_SERVER['REQUEST_METHOD'] === "POST") {
    ...
    if(empty($author_email) || empty($author_password)) {
       header("Location: log.php?message=Empty+Fields");
       exit();
    }
    ...
    if(!filter_var($author_email, FILTER_VALIDATE_EMAIL)) {
        header("Location: log.php?message=Please+Enter+A+Valid+Email");
        exit();
    } else {
        ...
        if(mysqli_num_rows($result) > 0) {
            while($row = mysqli_fetch_assoc($result)) {
                ...
                if (password_verify($author_password, $row['author_password'])) {
                    ...
                    header("Location: log.php?message=Login+Error");
                    exit();
                }
            }
        } else {
            // Email does not exist
            header("Location: log.php?message=Email+Not+Found");
            exit();
        }
    }
}
